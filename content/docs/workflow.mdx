...
0. **Init Guard** — automatically checks if the project is initialized; runs `/project-init` if needed. Also verifies `pactkit.yaml` (in `.claude/` or `.opencode/`) completeness and backfills missing sections.
