...
- **Iteration limit** — prevents infinite loops with bounded attempts
- **Full verification** — ensures entire test suite passes
...
The QA engineer verifies the implementation:

- Security review against common vulnerabilities
...
- **Hygiene** — all Board tasks checked off
...
This orchestrates all four phases automatically using a multi-agent team.

<Callout type="warning">
**Note**: `/project-sprint` is currently only available in Claude Code. OpenCode users should run the four phases individually.
</Callout>
