...
0. **Init Guard** — automatically checks if the project is initialized; runs `/project-init` if needed
1. **Visual Scan** — run `visualize` to understand current codebase structure
2. **Logic Trace** — if modifying existing logic, uses the `pactkit-trace` skill
...
4. **Lint Check** — run linter if CI config exists (e.g., `ruff check` for Python)
5. **Verify** — ensure the full test suite passes
...
3. **Lint Gate** — run linter if CI config exists; stop if lint fails
4. **Architecture Maintenance** — verify `system_design.mmd` component counts, refresh `rules.md` test invariants
5. **Hygiene** — confirm all Board tasks are checked off
6. **Archive** — move completed Stories to archive
7. **Release** (conditional) — version bump, snapshot, snapshot verification
8. **Commit** — conventional commit (`feat(scope): description`)
