...
├── rules/                    ← 8 rule files (3 core always-load + 6 on-demand)
...
The 8 rule modules form the "constitution" that governs agent behavior:

| Module | Type | Purpose |
|--------|------|---------|
| **Core Protocol** | Core | Session Context, Visual First, Strict TDD, Language Matching |
| **Hierarchy of Truth** | Core | Spec > Tests > Code precedence, conflict resolution rules |
| **File Atlas** | On-demand | Standard file locations and their purposes |
| **Routing Table** | On-demand | Maps commands to agents and playbooks |
| **Workflow Conventions** | On-demand | Conventional Commits, branch naming, PR conventions |
| **MCP Integration** | On-demand | Conditional MCP server usage by PDCA phase |
| **Shared Protocols** | On-demand | Cross-command protocols (Lazy Visualize, Test Mapping, Context.md format) |
| **Architecture Principles** | On-demand | SOLID/DRY patterns, architecture decision guidelines |

<Callout type="info">
**Lazy Loading (OpenCode)**: Core rules are loaded on every turn via `opencode.json` instructions. On-demand rules are referenced in `AGENTS.md` and loaded by the AI only when needed — reducing token consumption by ~62% per turn compared to loading all rules.
</Callout>
