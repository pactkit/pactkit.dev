...
P.A.C.T. was created by **Slim** in early 2026 as the governance foundation for [PactKit](https://pactkit.dev), an open-source toolkit for governing AI coding agents. The framework emerged from a practical observation: **most AI agent failures are boundary violations** — agents computing when they should delegate to code, or fabricating when they should read data.
