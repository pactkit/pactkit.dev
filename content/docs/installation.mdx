...
Restart Claude Code. All 9 agents, 11 commands, and 10 skills are immediately available.
...
| Path | Contents |
|------|----------|
| `CLAUDE.md` | Constitution root (references 7 rule modules via `@import`) |
...
| `rules/` | 7 modular rule files (loaded via `instructions: ["rules/*.md"]`) |
...
|---------|-------------|----------|
| Rules loading | `@import` in CLAUDE.md | `instructions: ["rules/*.md"]` in opencode.json |
